...
def get_json(url: str, headers: Optional[Dict[str, str]] = None, *,
    ...
    """``get`` + JSON decode. Returns ``(status, obj_or_None)``.
    ...
    """
    status, body = get(url, headers, timeout=timeout, retries=retries, sleep=sleep)
    if body is None:
        return status, None
    try:
...
def fetch(url: str, headers: Optional[Dict[str, str]] = None, *,
    ...
    """Body bytes, or None on any failure.

    Convenience only — prefer :func:`get` where "the server said no" and "we
    could not reach the server" must be told apart.
    """
    _status, body = get(url, headers, timeout=timeout, retries=retries, sleep=sleep)
    return body
...
def fetch_json(url: str, headers: Optional[Dict[str, str]] = None, *,
    ...
    """Parsed JSON, or None. See :func:`get_json` for the status-aware form."""
    _status, obj = get_json(url, headers, timeout=timeout, retries=retries, sleep=sleep)
    return obj
